...
try:
    ...
    db = firestore.client()
    print("✅ Firebase Admin başarıyla bağlandı! Çevrimdışı koruma aktif.")
...
class MiniPillWindow(QWidget):
    def __init__(self, scale_factor):
        ...
        self.setStyleSheet("QWidget { background: rgba(28, 32, 43, 0.95); border-radius: 12px; }")
        ...
        self.card_rem = QFrame(); self.setup_card(self.card_rem, "⏳", "00:00", "KALAN SÜRE", scale_factor, (0,0))
        self.card_best = QFrame(); self.setup_card(self.card_best, "⏱️", "--:--.---", "EN İYİ TUR SÜRESİ", scale_factor, (0,1))
        self.card_curr = QFrame(); self.setup_card(self.card_curr, "🔄", "--:--.---", "MEVCUT TUR SÜRESİ", scale_factor, (1,0))
        self.card_track = QFrame(); self.setup_card(self.card_track, "📍", "BEKLENİYOR...", "PİST / ARAÇ", scale_factor, (1,1))
    ...
    def setup_card(self, card, icon_text, value_text, label_text, scale_factor, grid_pos):
        ...
        card_lay = QHBoxLayout(card); card_lay.setContentsMargins(0, 0, 0, 0); card_lay.setSpacing(15)
        icon = QLabel(icon_text); icon.setStyleSheet(f"color: #ff3b3b; font-size: {int(24*scale_factor)}px; background: transparent;"); icon.setAlignment(Qt.AlignTop | Qt.AlignHCenter)
        v_lay = QVBoxLayout(); v_lay.setSpacing(0)
        value_label = QLabel(value_text); value_label.setStyleSheet(f"color: #ffffff; font-size: {int(32*scale_factor)}px; font-family: 'Roboto Condensed', sans-serif; background: transparent;")
        desc_label = QLabel(label_text); desc_label.setStyleSheet(f"color: #8a92a3; font-size: {int(12*scale_factor)}px; background: transparent; font-weight: bold; letter-spacing: 1px;")
        v_lay.addWidget(value_label); v_lay.addWidget(desc_label); v_lay.addStretch()
        card_lay.addWidget(icon, alignment=Qt.AlignTop); card_lay.addLayout(v_lay)
        self.main_lay.addWidget(card, grid_pos[0], grid_pos[1])
        card.value_label = value_label; card.desc_label = desc_label 
    ...
    def update_telemetry(self, track, car, curr, best, scale_factor):
        if track:
            self.card_track.value_label.setText(f"{car[:12]}"); self.card_track.desc_label.setText(f"{track[:20].upper()}")
            ...
            self.card_track.value_label.setText("BEKLENİYOR..."); self.card_track.desc_label.setText("PİST / ARAÇ")
            self.card_curr.value_label.setText("--:--.---"); self.card_best.value_label.setText("--:--.---")
...
class ACTelemetryWorker(QObject):
    ...
    def run(self):
        ...
        class ACStatic(ctypes.Structure):
            _pack_ = 4
            _fields_ = [("smVersion", ctypes.c_wchar * 15), ("acVersion", ctypes.c_wchar * 15), ("numberOfSessions", ctypes.c_int32), ("numCars", ctypes.c_int32), ("carModel", ctypes.c_wchar * 33), ("track", ctypes.c_wchar * 33)]
        class ACGraphics(ctypes.Structure):
            _pack_ = 4
            _fields_ = [("packetId", ctypes.c_int32), ("status", ctypes.c_int32), ("session", ctypes.c_int32), ("currentTime", ctypes.c_wchar * 15), ("lastTime", ctypes.c_wchar * 15), ("bestTime", ctypes.c_wchar * 15)]
        
        self.total_ms = 0; self.last_packet_id = -1
        
        while True:
            handle_static = kernel32.OpenFileMappingW(FILE_MAP_READ, False, "Local\\acpmf_static")
            if not handle_static:
                self.telemetry_updated.emit("", "", "", "", ""); self.total_ms = 0; self.last_packet_id = -1
            ...
            try:
                ...
                track_name = ACStatic.from_buffer_copy(shm_static).track; car_name = ACStatic.from_buffer_copy(shm_static).carModel
                g_data = ACGraphics.from_buffer_copy(shm_graphics)
                
                if g_data.status == 2 and track_name:
                    if self.last_packet_id != -1 and g_data.packetId > self.last_packet_id: self.total_ms += 500
                ...
                shm_static.close(); shm_graphics.close()
            except Exception: 
                self.telemetry_updated.emit("", "", "", "", ""); self.total_ms = 0; self.last_packet_id = -1
    ...
    def ms_to_time_string(self, ms):
        seconds = ms // 1000; hours = seconds // 3600; seconds %= 3600; minutes = seconds // 60; seconds %= 60
...
class GameCard(QFrame):
    ...
    def __init__(self, title, img_url, local_path, loader, scale_factor, parent=None):
        ...
        self.local_path = local_path; self.scale_factor = scale_factor
        base_width, base_height = int(520 * self.scale_factor), int(780 * self.scale_factor)
        self.base_poster_size = QSize(base_width, base_height); self.hover_poster_size = QSize(int(base_width * 1.1), int(base_height * 1.1))
    ...
    def enterEvent(self, event):
        cw, ch = self.poster_container.width(), self.poster_container.height(); w, h = self.hover_poster_size.width(), self.hover_poster_size.height()
    ...
    def leaveEvent(self, event):
        cw, ch = self.poster_container.width(), self.poster_container.height(); w, h = self.base_poster_size.width(), self.base_poster_size.height()
        self.anim.setEndValue(QRect((cw - w) / 2, (ch - h) / 2, w, h)); self.anim.start()
        self.poster.setStyleSheet("background: #1a1a1a; border-radius: 25px; border: 3px solid #333;")
...
class SpeedPointAgent(QWidget):
        ...
        def _update():
            try:
                ...
                if not self.is_offline:
                    print(f"\n⚠️ İNTERNET KESİNTİSİ! Oyuncu etkilenmeyecek, süre bilgisayarda sayılmaya devam ediyor...")
    ...
    def handle_telemetry(self, track, car, curr, best, total):
        if best and best != "0" and best != "--:--.---":
            self.session_track = track; self.session_car = car; self.session_best_time = best; self.session_total_time = total
            if self.session_best_time != self.last_sent_best_time:
                self.last_sent_best_time = self.session_best_time; self.save_session_record() 
    ...
    def save_session_record(self):
        if self.session_best_time and self.session_best_time != "0" and self.session_doc_id:
            payload = {"userId": self.current_user_id, "userName": self.current_user_name, "carModel": self.session_car, "track": self.session_track, "bestTime": self.session_best_time, "totalRaceTime": self.session_total_time}
            def _patch():
                try: db.collection("leaderboard").document(self.session_doc_id).set(payload)
                except: pass
    ...
    def setup_locked_view(self):
        ...
        self.v_widget = QVideoWidget(); self.player = QMediaPlayer(); self.audio = QAudioOutput(); self.audio.setVolume(0); self.player.setAudioOutput(self.audio); self.player.setVideoOutput(self.v_widget)
        ...
        lay.addWidget(self.v_widget, 0, 0); lay.addWidget(scrim, 0, 0); lay.addWidget(self.lock_text, 0, 0); self.full_ui.addWidget(self.locked_widget)
    ...
    def setup_active_view(self):
        ...
        if os.path.exists(b_path): self.bg_label.setPixmap(QPixmap(b_path).scaled(QApplication.primaryScreen().size(), Qt.KeepAspectRatioByExpanding, Qt.SmoothTransformation)); blur = QGraphicsBlurEffect(); blur.setBlurRadius(250); self.bg_label.setGraphicsEffect(blur)
        ...
        v_header = QVBoxLayout(); self.welcome_label = QLabel("HOŞ GELDİN"); self.welcome_label.setStyleSheet(f"color: white; font-size: {int(36*self.scale_factor)}px; font-weight: bold;")
        ...
        self.grid = QGridLayout(); self.grid.setSpacing(80 * self.scale_factor); self.grid.setAlignment(Qt.AlignCenter); self.grid_container.addLayout(self.grid); self.scroll.setWidget(self.scroll_content); ui_lay.addWidget(self.scroll)
        lay.addWidget(self.bg_label, 0, 0); lay.addWidget(self.scrim_overlay, 0, 0); lay.addWidget(ui_frame, 0, 0); self.full_ui.addWidget(self.active_widget)
    ...
    def setup_loading_view(self):
        ...
        lay.addStretch(); lay.addWidget(logo, alignment=Qt.AlignCenter); lay.addSpacing(50); lay.addWidget(text, alignment=Qt.AlignCenter); lay.addStretch(); self.full_ui.addWidget(self.loading_widget)
    ...
    def setup_admin_panel(self):
        ...
        b_lay.addWidget(QLabel("ADMIN PIN", styleSheet="color: white; font-weight: bold;")); b_lay.addWidget(self.pin_input); b_lay.addWidget(btn); lay.addWidget(box, 0, Qt.AlignCenter)

    def switch_to_full(self):
        self.is_mini_mode = False; self.mini_window.hide(); self.showNormal(); self.showFullScreen(); self.full_ui.setCurrentIndex(1 if not self.is_locked else 0)

    def kill_current_game(self):
        if self.current_game_exe and sys.platform == "win32":
            try: subprocess.run(["taskkill", "/F", "/IM", self.current_game_exe, "/T"], capture_output=True); subprocess.run(["taskkill", "/F", "/IM", "acs.exe", "/T"], capture_output=True)
            except: pass
            self.current_game_exe = None

    def sync_status(self, locked_cloud, time_cloud, user_name, user_id):
        self.current_user_name = user_name.upper(); self.current_user_id = user_id 
        self.welcome_label.setText(f"HOŞ GELDİN, {self.current_user_name}")
        
        if self.is_locked == True and locked_cloud == False:
            self.is_locked = False; self.remaining_seconds = time_cloud; self.cloud_time_checkpoint = time_cloud
            ...
            self.is_locked = True; self.remaining_seconds = 0; self.cloud_time_checkpoint = 0
            ...
            if self.cloud_time_checkpoint != -1 and time_cloud != self.cloud_time_checkpoint:
                ...
                if delta != 0:
                    ...
                    print(f"\n🔄 Admin Müdahalesi / Senkronizasyon: {delta} saniye eklendi/çıkarıldı.")
                self.cloud_time_checkpoint = time_cloud
                ...
                def _force_lock():
                    while self.is_locked:
                        try:
                            ...
                            print("\n🔒 Süre bitti, bilgisayar ve bulut başarıyla kilitlendi.")
                            break
                        except:
                            print("\n⚠️ İnternet yok! Kilit emri bekletiliyor, 5 saniye sonra tekrar denenecek...")
                            time.sleep(5)
                            
                threading.Thread(target=_force_lock, daemon=True).start()

    def render_games(self, games):
        while self.grid.count(): item = self.grid.takeAt(0); item.widget().deleteLater()
    ...
    def launch_game(self, path):
        ...
        screen_geo = QApplication.primaryScreen().geometry(); pill_w, pill_h = self.mini_window.width(), self.mini_window.height(); self.mini_window.move(screen_geo.width() - pill_w - 20, 20); self.mini_window.show()
        ...
        try:
            ...
            self.switch_to_full(); print(f"❌ Oyun başlatılamadı: {e}"); return

        QTimer.singleShot(12000, self.hide_shield)

    def hide_shield(self):
        if self.is_mini_mode and not self.is_locked: self.hide() 
    ...
    def keyPressEvent(self, event):
        if event.key() == Qt.Key_X and (event.modifiers() & Qt.ControlModifier or event.modifiers() & Qt.MetaModifier) and (event.modifiers() & Qt.ShiftModifier): QApplication.quit()
        if event.key() == Qt.Key_Escape and self.is_mini_mode: self.kill_current_game(); self.switch_to_full()
        if event.key() == Qt.Key_M and (event.modifiers() & Qt.ControlModifier or event.modifiers() & Qt.MetaModifier) and (event.modifiers() & Qt.ShiftModifier): self.admin_overlay.setGeometry(0, 0, self.width(), self.height()); self.admin_overlay.setVisible(True); self.pin_input.setFocus()
